...
namespace NPOL.App_Code.Data
{
    public class EmpFeedbackDAO : BaseDAO
    {
        ...
        private tbl_EmpFeedback CreateNewsFromReader(IDataReader reader)
        {
            ...
            if (string.IsNullOrEmpty(reader["TrainingDetail"].ToString()))
                news.TrainingDetail = "";
            else
                news.TrainingDetail = reader["TrainingDetail"].ToString();

            if (!string.IsNullOrEmpty(reader["LineName"].ToString()))
                news.LineName = reader["LineName"].ToString();
            ...
            if (!string.IsNullOrEmpty(reader["TeamName"].ToString()))
                news.TeamName = reader["TeamName"].ToString();

            if (!string.IsNullOrEmpty(reader["LocationName"].ToString()))
                news.LocationName = reader["LocationName"].ToString();
