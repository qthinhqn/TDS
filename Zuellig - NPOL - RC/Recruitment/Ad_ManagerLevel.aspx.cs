...
namespace NPOL
{
    public partial class RC_ManagerLevel : System.Web.UI.Page
    {
        ...
        protected void btnTransfer_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    provider.CommandText = "Insert into tblRecruitManagerLevel_tmp (EmployeeID, DateChange, SectionID, LineID, PosID, CheckerID, ManagerID_L1, ManagerID_L2, ManagerID_L3) values(@EmployeeID, @DateChange, @SectionID, @LineID, @PosID, @CheckerID, @ManagerID_L1, @ManagerID_L2, @ManagerID_L3)";
                    provider.ParameterCollection = new string[] { "@EmployeeID", "@DateChange", "@SectionID", "@LineID", "@PosID", "@CheckerID", "@ManagerID_L1", "@ManagerID_L2", "@ManagerID_L3" };
                    ...
                    object checkerID = dt.Rows[i]["CheckerID"];
                    object managerid_l1 = dt.Rows[i]["ID_L1"];
                    object managerid_l2 = dt.Rows[i]["ID_L2"];
                    object managerid_l3 = dt.Rows[i]["ID_L3"];
                    provider.ValueCollection = new object[] { empid, datechange, sectionid, lineid, posid, checkerID, managerid_l1, managerid_l2, managerid_l3 };
        ...
        private DataTable EmployeeInfo(object EmpID)
        {
            ...
            khSqlServerProvider provider = new khSqlServerProvider(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["ZuelligConnection"].ConnectionString);
            provider.CommandText = "Select * from view_DinhCapRecruit where EmployeeID = @EmployeeID";
        ...
        {
            khSqlServerProvider provider = new khSqlServerProvider(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["ZuelligConnection"].ConnectionString);
            provider.CommandText = "Delete from tblRecruitManagerLevel_tmp";
            provider.ExecuteNonQuery();
            provider.CloseConnection();
        }
        ...
        private DataTable CollectDataSource()
        {
            ...
            for (int i = 0; i < gvDinhCap.VisibleRowCount; i++)
            {
                ...
                if (dtDinhCap.Rows.Count > 0)
                {
                    ...
                    arr.Add(dtDinhCap.Rows[0]["CheckerID"]);
            ...
            try
            {
                ...
                if (exceldata.Rows.Count > 0)
                {
                    for (int i = 0; i < exceldata.Rows.Count; i++)
                    {
                        ...
                        provider.CommandText = "Insert into tblRecruitManagerLevel_tmp (EmployeeID, DateChange, SectionID, LineID, PosID, ManagerID_L1, ManagerID_L2, ManagerID_L3) values(@EmployeeID, getdate(), @SectionID, @LineID, @PosID, @ManagerID_L1, @ManagerID_L2, @ManagerID_L3)";
        ...
        protected void gvDinhCapTMP_CustomColumnDisplayText(object sender, ASPxGridViewColumnDisplayTextEventArgs e)
        {
            ...
            object CheckerID = e.GetFieldValue("CheckerID");
            ...
            if (e.GetFieldValue("EmployeeID") != null)
            {
                if (!EmpID.ToString().Equals(""))
                {
                    ...
                    if (e.Column.FieldName.Equals("CheckerID"))
                    {
                        e.DisplayText = getEmployeeName(CheckerID.ToString());
                    }
        ...
        private void UpdateManagerLevel()
        {
            ...
            khSqlServerProvider provider = new khSqlServerProvider(System.Web.Configuration.WebConfigurationManager.ConnectionStrings["ZuelligConnection"].ConnectionString);
            provider.CommandText = "Select * from tblRecruitManagerLevel_tmp";
            DataTable dt = provider.GetDataTable();
            if (dt.Rows.Count == 0)
            {
            ...
            else if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (!kh.IsExistsEmployeeRecruit(employeeid.ToString()))
                    {
                        //Thêm mới định cấp
                        provider.CommandText = "Insert into tblRecruitManagerLevel (EmployeeID, DateChange, SectionID, LineID, PosID, CheckerID, ManagerID_L1, ManagerID_L2, ManagerID_L3) values(@EmployeeID, getdate(), @SectionID, @LineID, @PosID, @CheckerID, @ManagerID_L1, @ManagerID_L2, @ManagerID_L3)";
                        provider.ParameterCollection = new string[] { "@EmployeeID", "@SectionID", "@LineID", "@PosID", "@CheckerID", "@ManagerID_L1", "@ManagerID_L2", "@ManagerID_L3" };
                        provider.ValueCollection = new object[] { employeeid, sectionid, lineid, posid, checkerID, m1, m2, m3 };
                        provider.ExecuteNonQuery();
                    }
                    else
                    {
                        //Cập nhật định cấp cho tblRecruitManagerLevel
                        provider.CommandText = "Update tblRecruitManagerLevel Set CheckerID=@CheckerID, ManagerID_L1 = @L1, ManagerID_L2 = @L2, ManagerID_L3 = @L3 where EmployeeID = @EmpID";
                        provider.ParameterCollection = new string[] { "@L1", "@L2", "@L3", "@CheckerID", "@EmpID" };
                        provider.ValueCollection = new object[] { m1, m2, m3, checkerID, employeeid };
        ...
        protected void btnView_Click(object sender, EventArgs e)
        {
            ...
            object empid = grid.GetRowValues(container.VisibleIndex, new string[] { "EmployeeID" });

            //Xử lý giá trị biến
            empid = (empid == null ? DBNull.Value : empid);

            //Gán dữ liệu vào Popup control
            ASPxPopupControl1.ShowOnPageLoad = true;
            gvOTList.DataSource = Recruit_EmpGroupService.GetTableByid(empid.ToString());
            gvOTList.DataBind();
