...
namespace Canteen.CantinTHP.Endpoints
{
    ...
    using MyRepository = Repositories.TbEmpTypeRepository;
    using MyRow = Entities.TbEmpTypeRow;

    [RoutePrefix("Services/CantinTHP/TbEmpType"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TbEmpTypeController : ServiceEndpoint
    {
        ...
        {
            string file = "Template_EmpType.xlsx";
        ...
        public ExcelImportResponse ExcelImport(IUnitOfWork uow, ExcelImportRequest request)
        {
            ...
            for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
            {
                try
                {
                    ...
                    var typeid = Convert.ToInt32(worksheet.Cells[row, 3].Value ?? "0");
                    ...
                    var al = uow.Connection.TryFirst<MyRow>(q => q
                        .Select(p.EmpId, p.TypeId, p.DateChange, p.Remarks, p.KeyId)
                        .Where(p.EmpId == EmployeeID && p.TypeId == typeid && p.DateChange == datechange.Value));
