...
namespace Canteen.CantinTHP.Endpoints
{
    ...
    using MyRepository = Repositories.TbEmpCostCenterRepository;
    using MyRow = Entities.TbEmpCostCenterRow;

    [RoutePrefix("Services/CantinTHP/TbEmpCostCenter"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TbEmpCostCenterController : ServiceEndpoint
    {
        ...
        {
            string file = "Template_EmpCostCenter.xlsx";
            ...
            for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
            {
                try
                {
                    ...
                    var al = uow.Connection.TryFirst<MyRow>(q => q
                        .Select(p.EmpId, p.CostCenter, p.DateChange, p.Remarks, p.KeyId)
                        .Where(p.EmpId == EmployeeID && p.CostCenter == costcenter && p.DateChange == datechange.Value));
