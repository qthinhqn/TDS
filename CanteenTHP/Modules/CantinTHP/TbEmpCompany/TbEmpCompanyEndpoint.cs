...
namespace Canteen.CantinTHP.Endpoints
{
    ...
    using MyRepository = Repositories.TbEmpCompanyRepository;
    using MyRow = Entities.TbEmpCompanyRow;

    [RoutePrefix("Services/CantinTHP/TbEmpCompany"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TbEmpCompanyController : ServiceEndpoint
    {
        ...
        {
            string file = "Template_EmpCompany.xlsx";
        ...
        public ExcelImportResponse ExcelImport(IUnitOfWork uow, ExcelImportRequest request)
        {
            ...
            for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
            {
                try
                {
                    ...
                    var company = Convert.ToString(worksheet.Cells[row, 3].Value ?? "");
                    ...
                    var al = uow.Connection.TryFirst<MyRow>(q => q
                        .Select(p.EmpId, p.CompanyKey, p.DateChange, p.Remarks, p.KeyId)
                        .Where(p.EmpId == EmployeeID && p.CompanyKey == company && p.DateChange == datechange.Value));
