...
namespace Canteen.CantinTHP.Endpoints
{
    ...
    using MyRepository = Repositories.TbEmpCardRepository;
    using MyRow = Entities.TbEmpCardRow;

    [RoutePrefix("Services/CantinTHP/TbEmpCard"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TbEmpCardController : ServiceEndpoint
    {
        ...
        {
            string file = "Template_EmpCard.xlsx";
        ...
        public ExcelImportResponse ExcelImport(IUnitOfWork uow, ExcelImportRequest request)
        {
            ...
            for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
            {
                try
                {
                    ...
                    var CardID = Convert.ToString(worksheet.Cells[row, 3].Value ?? "");
                    if (CardID.IsTrimmedEmpty())
                        break;
                    ...
                    var al = uow.Connection.TryFirst<MyRow>(q => q
                        .Select(p.EmpId, p.CardId, p.DateChange, p.Remarks, p.Active, p.KeyId)
                        .Where(p.EmpId == EmployeeID && p.CardId == CardID && p.DateChange == datechange.Value));
