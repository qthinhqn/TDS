...
namespace Canteen.CantinTHP.Endpoints
{
    ...
    using MyRepository = Repositories.TbEmpCostTempRepository;
    using MyRow = Entities.TbEmpCostTempRow;

    [RoutePrefix("Services/CantinTHP/TbEmpCostTemp"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TbEmpCostTempController : ServiceEndpoint
    {
        ...
        {
            string file = "Template_EmpCostTemp.xlsx";
        ...
        public ExcelImportResponse ExcelImport(IUnitOfWork uow, ExcelImportRequest request)
        {
            ...
            for (var row = 2; row <= worksheet.Dimension.End.Row; row++)
            {
                try
                {
                    ...
                    var costcenter = Convert.ToString(worksheet.Cells[row, 3].Value ?? "");
                    if (costcenter.IsTrimmedEmpty())
                        break;
                    ...
                    var shift = Convert.ToString(worksheet.Cells[row, 5].Value ?? "");

                    var al = uow.Connection.TryFirst<MyRow>(q => q
                        .Select(p.EmpId, p.CostId, p.DDay, p.ShiftId, p.KeyId)
                        .Where(p.EmpId == EmployeeID && p.CostId == costcenter && p.DDay == datechange.Value));
