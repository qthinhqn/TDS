...
namespace Canteen.CantinTHP.Endpoints
{
    ...
    using MyRepository = Repositories.TbEmpManagerRepository;
    using MyRow = Entities.TbEmpManagerRow;

    [RoutePrefix("Services/CantinTHP/TbEmpManager"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class TbEmpManagerController : ServiceEndpoint
    {
        ...
        {
            string file = "Template_EmpManager.xlsx";
                ...
                try
                {
                    ...
                    try
                    {
                        if (worksheet.Cells[row, 4].Value != null)
                            isManager = Convert.ToBoolean(worksheet.Cells[row, 4].Value);
                    }
                    catch { }
                    ...
                    var al = uow.Connection.TryFirst<MyRow>(q => q
                        .Select(p.EmpId, p.IsManager, p.DateChange, p.Remarks, p.KeyId)
                        .Where(p.EmpId == EmployeeID && p.IsManager == (isManager.Value ? 1 : 0) && p.DateChange == datechange.Value));
